...
// 
// So we use a primitive logging macro around MYLog.
// We maintain the NS prefix on the macros to be explicit about the fact that we're using MYLog.

#define DD_DEBUG NO

#define MYLogDebug(frmt, ...) do{ if(DD_DEBUG) MYLog((frmt), ##__VA_ARGS__); } while(0)
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		MYLogDebug(@"DDLog: Using grand central dispatch");
		...
		MYLogDebug(@"DDLog: numProcessors = %u", numProcessors);
...
+ (void)lt_removeLogger:(id <DDLogger>)logger
{
	...
	if (loggerNode == nil)
	{
		MYLogDebug(@"DDLog: Request to remove logger which wasn't added");
		return;
	}
