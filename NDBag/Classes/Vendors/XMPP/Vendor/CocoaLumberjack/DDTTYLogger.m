...
// 
// So we use primitive logging macros around MYLog.
// We maintain the NS prefix on the macros to be explicit about the fact that we're using MYLog.

#define LOG_LEVEL 2

#define MYLogError(frmt, ...)    do{ if(LOG_LEVEL >= 1) MYLog((frmt), ##__VA_ARGS__); } while(0)
#define MYLogWarn(frmt, ...)     do{ if(LOG_LEVEL >= 2) MYLog((frmt), ##__VA_ARGS__); } while(0)
#define MYLogInfo(frmt, ...)     do{ if(LOG_LEVEL >= 3) MYLog((frmt), ##__VA_ARGS__); } while(0)
#define MYLogVerbose(frmt, ...)  do{ if(LOG_LEVEL >= 4) MYLog((frmt), ##__VA_ARGS__); } while(0)
...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor
{
	...
	for (OSColor *color in colors)
	{
		...
		MYLogVerbose(@"DDTTYLogger: %3lu : %.3f,%.3f,%.3f & %.3f,%.3f,%.3f = %.6f",
		...
		if (distance < lowestDistance)
		{
			...
			MYLogVerbose(@"DDTTYLogger: New best index = %lu", (unsigned long)bestIndex);
		}
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		MYLogInfo(@"DDTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
		MYLogInfo(@"DDTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
		MYLogInfo(@"DDTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		MYLogInfo(@"DDTTYLogger: newColorProfile: %@", newColorProfile);
...
- (void)logMessage:(DDLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		{
			// The log message is unformatted, so apply standard MYLog style formatting.
